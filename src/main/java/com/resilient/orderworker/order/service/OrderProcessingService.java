...
/**
 * Main service for processing orders.
 * Coordinates enrichment, validation, and storage of orders.
 */
@Service
public class OrderProcessingService {
    ...
    public Mono<Order> processOrder(OrderMessage orderMessage) {
        ...
        return lockService.executeWithLock(orderMessage.orderId(), () -> 
    ...
    private Mono<Order> processOrderInternal(OrderMessage orderMessage) {
        ...
        return orderRepository.existsByOrderId(orderMessage.orderId())
            .flatMap(exists -> {
                if (exists) {
            ...
            .doOnSuccess(order -> logger.info("Successfully processed order: {}", orderMessage.orderId()))
            .doOnError(error -> logger.error("Error processing order {}: {}", 
    ...
    private Mono<Order> enrichAndSaveOrder(OrderMessage orderMessage) {
        ...
        Mono<CustomerResponse> customerMono = customerService.getCustomer(orderMessage.customerId());
        
        List<String> productIds = orderMessage.products().stream()
     ...
     */
    private Mono<Order> validateAndBuildOrder(OrderMessage orderMessage, 
        ...
        if (!allProductsValid) {
            return Mono.error(new OrderProcessingException(
                "One or more products are invalid for order: " + orderMessage.orderId()));
        }
        
        // Build order products with quantities
        List<Order.OrderProduct> orderProducts = orderMessage.products().stream()
            .map(productInfo -> {
                ProductResponse product = products.stream()
                    .filter(p -> p.productId().equals(productInfo.productId()))
                    .findFirst()
                    .orElseThrow(() -> new OrderProcessingException(
                        "Product not found: " + productInfo.productId()));
        ...
        double totalAmount = orderProducts.stream()
            .mapToDouble(Order.OrderProduct::getSubtotal)
            .sum();
        ...
        logger.debug("Built order with total amount: {} for order: {}", 
            totalAmount, orderMessage.orderId());
