 ...
 * Prevents concurrent processing of the same order across multiple instances.
 */
@Service
    ...
    public <T> Mono<T> executeWithLock(String orderId, Supplier<Mono<T>> task, 
        ...
        return Mono.fromCallable(() -> {
            ...
            try {
                ...
                if (!acquired) {
                    logger.warn("Failed to acquire lock for order: {} within {} seconds", orderId, waitTime);
                ...
                throw new RuntimeException("Interrupted while acquiring lock for order: " + orderId, e);
            }
        })
        .subscribeOn(Schedulers.boundedElastic())
        .flatMap(lock -> task.get()
            .doFinally(signalType -> {
                try {
                    if (lock.isHeldByCurrentThread()) {
                        lock.unlock();
                        logger.debug("Released lock for order: {}", orderId);
                    }
                } catch (Exception e) {
                    logger.error("Error releasing lock for order {}: {}", orderId, e.getMessage());
                }
            }));
    }
